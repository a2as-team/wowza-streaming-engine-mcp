...
class WowzaMediaCasterApiClient:
    """
    Asynchronous API client for interacting with the Wowza Streaming Engine REST API.
    This client is specifically tailored for MediaCaster management.
    """
    def __init__(self, base_url: str, username: Optional[str] = None, password: Optional[str] = None, logger=None):
        ...
        self.logger("info", f"WowzaApiC (Media Caster) initialized for base URL: {self.base_url}")
    ...
    # --- Server MediaCaster Management (v2) ---
    async def get_media_casters(self, server_name: str) -> Dict:
        """
        ...
        Returns:
            Dict: A dictionary containing the list of MediaCaster configurations.
        """
        url = f"{self.base_url}/v2/servers/{server_name}/mediacasters"
        self.logger("info", f"Fetching MediaCasters for server: {server_name}")
        return await self._make_request("GET", url)

    async def get_media_caster(self, server_name: str, mediacaster_name: str) -> Dict:
        """
        Retrieves the specified MediaCaster configuration.

        Args:
            server_name (str): The name of the server instance.
            mediacaster_name (str): The name of the MediaCaster.

        Returns:
            Dict: A dictionary containing the specified MediaCaster's configuration.
        """
        url = f"{self.base_url}/v2/servers/{server_name}/mediacasters/{mediacaster_name}"
        self.logger("info", f"Fetching MediaCaster '{mediacaster_name}' for server: {server_name}")
        return await self._make_request("GET", url)