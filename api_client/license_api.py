...
class WowzaLicenseApiClient:
    """
    Asynchronous API client for interacting with the Wowza Streaming Engine REST API.
    This client is specifically tailored for server license management.
    """
    def __init__(self, base_url: str, username: Optional[str] = None, password: Optional[str] = None, logger=None):
        ...
        self.logger("info", f"WowzaApiC (License) initialized for base URL: {self.base_url}")
        ...
        try:
            async with aiohttp.ClientSession() as session:
                ...
                async with session.request(method, url, headers=self.headers, params=params, json=data, auth=auth) as response:
                    ...
                    # A successful PUT often returns 200/OK with a body or 204/No Content
    ...
    # --- Server License Management (v2) ---
    async def get_server_licenses(self, server_name: str) -> Dict:
        """
        ...
        Returns:
            Dict: A dictionary containing the server license configuration.
        """
        url = f"{self.base_url}/v2/servers/{server_name}/licenses"
        self.logger("info", f"Fetching licenses for server: {server_name}")
        return await self._make_request("GET", url)

    async def update_server_licenses(self, server_name: str, data: Dict) -> Dict:
        """
        Updates the Server License list.

        Args:
            server_name (str): The name of the server instance (e.g., '_defaultServer_').
            data (Dict): The new license configuration payload.

        Returns:
            Dict: The result of the update operation.
        """
        url = f"{self.base_url}/v2/servers/{server_name}/licenses"
        self.logger("info", f"Updating licenses for server: {server_name}")
