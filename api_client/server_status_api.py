# api_client/server_status_api.py
import aiohttp
import logging
from typing import Dict, Optional

class WowzaServerStatusApiClient:
    """
    Asynchronous API client for interacting with the Wowza Streaming Engine REST API.
    This client is specifically tailored for retrieving server status.
    """
    def __init__(self, base_url: str, username: Optional[str] = None, password: Optional[str] = None, logger=None):
        ...
        self.logger("info", f"WowzaApiC (Server Status) initialized for base URL: {self.base_url}")
    ...
    async def get_server_status(self, server_name: str) -> Dict:
        ...
        Returns:
            Dict: A dictionary containing detailed status information about the server.
        """
        url = f"{self.base_url}/v2/servers/{server_name}/status"
        self.logger("info", f"Fetching status for server: {server_name}")
